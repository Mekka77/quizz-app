...
@section('title', 'Dodaj pytanie do quizu: ' . $quiz->title)

@section('content')
<div class="container">
    <h1 class="fw-bold mb-4">Dodaj pytanie do quizu: "{{ $quiz->title }}"</h1>
        ...
        <div class="card-body">
            <form action="{{ route('admin.quizzes.questions.store', $quiz) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('text') is-invalid @enderror" id="question_text" name="text" value="{{ old('text') }}" required>
                    @error('text')
                ...
                <div id="answers-container">
                    @php $oldAnswers = old('answers', [null, null, null, null]); @endphp
                    @foreach($oldAnswers as $index => $oldAnswer)
                        <div class="input-group mb-3 answer-item">
                            <div class="input-group-text">
                                <input class="form-check-input mt-0 @error('answers.' . $index . '.is_correct') is-invalid @enderror" type="checkbox" name="answers[{{ $index }}][is_correct]" value="1" @checked(old('answers.' . $index . '.is_correct', false)) aria-label="Odpowiedź poprawna">
                            </div>
                            <input type="text" class="form-control @error('answers.' . $index . '.text') is-invalid @enderror" name="answers[{{ $index }}][text]" placeholder="Treść odpowiedzi" value="{{ old('answers.' . $index . '.text', '') }}" required>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (answersContainer.children.length === 0) {
            addAnswerField();
            addAnswerField();
        } else if (answersContainer.children.length < 2) {
